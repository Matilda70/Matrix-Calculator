...
// bindings_capi.cpp — C API для Emscripten (без Embind), работает с cwrap()


extern "C" {

// forward-declarations из operations.cpp



// --- Создание/уничтожение и доступ к элементам ---
...
// --- Доп. операции ---
Matrix* power_m(Matrix* a, int exp) {           // exp — int (i32 в wasm)
    Matrix copy = *a;
    Matrix res  = ::power(copy, (long long)exp);  // если ::power ждёт long long
    return new Matrix(std::move(res));
}
int rank_m(Matrix* a) {
    Matrix copy = *a;
